...
* Benjamin Carlson, Josh Magalhaes, Erik Marrro, Nick Sbabo
*
...
int trophy_score = 0;		// value of the trophy
int trophy_row = 0;		// row of the trophy
int trophy_col = 0;		// column of the trophy
int trophy_lifespanthirdstrike = 0;	// helper variable for lifespan
int trophy_lifespan = 0;	// lifespan of the trophy
// end variables *************
...
main ()
{
  ...
  void trophy ()		/* Initializes the trophy and places it on the screen by Joshua Magalhaes */
...
*/
void
init_snake (int start_y, int start_x)
{
...
run_when_time_runs_out ()
{
  // if snake head is on the trophy, grow snake by trophy amount and call trophy again
  if (head->row == trophy_row && head->column == trophy_col)
    {
      grow_snake (trophy_score);
      trophy();
    }
...
detect_collisions ()
{
  ...
  if (head->row == 1 || head->row == LINES - 1 || head->column == 0
      || head->column == COLS - 1)
    {
...
void
trophy()/* Trophy Spawning System by Joshua Magalhaes */
{
    trophy_col = rand() % COLS;
    trophy_row = rand() % LINES;
    trophy_score = rand() % 9;
...
/* Trophy Refresh system by Joshua Magalhaes */
void
trophy_refresh ()
{
  if (trophy_lifespan == 0)
    {
      move (trophy_row, trophy_col);
      printw (" ");
      trophy ();
